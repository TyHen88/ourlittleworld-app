...
import { Bot, LifeBuoy, Palette, Send, Trash2, X, Zap, Award } from "lucide-react";
...
function TypewriterContent({
  ...
  return (
    <div
      className="prose prose-sm max-w-none prose-slate normal-case prose-p:my-2 prose-headings:text-slate-900 prose-strong:text-slate-900 prose-headings:mt-4 prose-headings:mb-2 prose-ul:my-2 prose-li:my-1"
...
export function AIAdvisorChat({
  ...
  const handleSend = async () => {
    ...
    const nextMessages = messages
      .map((message): Message => ({ ...message, isNew: false }))
      .concat(userMsg);
  ...
  const markAsOld = (index: number) => {
    setMessages((prev) =>
      prev.map((message, messageIndex) =>
        messageIndex === index ? { ...message, isNew: false } : message
      )
    );
  };
  ...
  return (
    <div
      className={cn(
        ...
        isSheet
          ? "max-w-xl rounded-t-[1.5rem] shadow-2xl max-h-[90vh]"
          : "rounded-[2rem] border border-slate-200/70 shadow-xl min-h-[70vh]"
      )}
    >
      {isSheet && (
      ...
      {!isSheet && (
        <div className="px-6 pt-6">
          <div className="rounded-3xl border border-slate-200 bg-slate-50/70 px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">Temporary Replacement</p>
            <p className="mt-1 text-sm font-medium text-slate-600">
              Goals is paused for now, so the toolbar opens your AI advisor instead.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="flex justify-start">
              <div className="p-4 bg-white rounded-[1.5rem] rounded-tl-none border border-slate-100 shadow-sm">
                <div className="flex gap-1.5">
                  ...
                  <div
                    className={cn(
                      "w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0.2s]",
                      isSingle ? "bg-emerald-400" : "bg-indigo-400"
                    )}
                  />
                  <div
                    className={cn(
                      "w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0.4s]",
                      isSingle ? "bg-emerald-400" : "bg-indigo-400"
                    )}
                  />
                </div>
